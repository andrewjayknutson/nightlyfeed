...
namespace NightlyRouteToSlack.Utilities
{
    public class JunkUtilities
    {
        ...
        public void RunGJImportEmployeeHoursWorked()
        {
            //string nowDate = "5/31/2022";
            string nowDate = DateTime.Now.AddDays(-1).ToShortDateString();
            ConfigSettings cs = new ConfigSettings();
            JunkDailySchedule js = new JunkDailySchedule();
            ...
            cmd.CommandText = "spReport_GetHoursWorkedByEmployee";

            cmd.Parameters.Add("@startDate", SqlDbType.DateTime);
            cmd.Parameters["@startDate"].Value = nowDate;

            cmd.Parameters.Add("@endDate", SqlDbType.NVarChar);
            cmd.Parameters["@endDate"].Value = nowDate;
            ...
            {
                js = new JunkDailySchedule();

                //date
                js.EmpStartTime = nowDate;

                //employee name
                js.EmpName = dr["employeeName"].ToString();

                //hours worked
                js.EmpWorkType = dr["countHours"].ToString();
            ...
            reader.ImportDailyEmployeeHoursWorked("18XB2NY_Gruimmn0lurBqBc4elAbJNiVDLyhdb-y9gWk", jds);


        }


        public void RunGJDailyEmployeeJobsDone() 
        {
            ConfigSettings cs = new ConfigSettings();
            JunkDailySchedule js = new JunkDailySchedule();
            List<JunkDailySchedule> jds = new List<JunkDailySchedule>();
            ...
            cmd.Connection = dc.conn;
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.CommandText = "spReport_GetJobsByEmployee";

            cmd.Parameters.Add("@startDate", SqlDbType.DateTime);
            cmd.Parameters["@startDate"].Value = DateTime.Now.ToShortDateString();

            cmd.Parameters.Add("@endDate", SqlDbType.NVarChar);
            cmd.Parameters["@endDate"].Value = DateTime.Now.ToShortDateString();
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //date
                js.EmpStartTime = DateTime.Now.ToShortDateString();

                //employee id
                js.EmpPhone = dr["employeeID"].ToString();

                //employee name
                js.EmpName = dr["employeeName"].ToString();

                //jobs done
                js.EmpWorkType = dr["countJobs"].ToString();
            ...
            GoogleSpreadSheetReader reader = new GoogleSpreadSheetReader(googleService);
            reader.ImportDailyEmployeeJobDone("1y_4WS7hQ4NbJ-yQ1yY7pVi5qr3Ww4cMNnc1tUwILnWw", jds);
        ...
        public void RunSalesLeadAJS()
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                if (float.Parse(dr["AJS"].ToString()) < 450)
                {
                    if (firstOne)
                    {
                        firstOne = false;
        ...
        public void JunkScorecardDaily()
        {
            ...
            float yesterdaySGAPercentage = 0;
            float yesterdayDumpPercentage = 0;
            ...
            float mtdWarehouseWage = 0;
            float mtdWarehouseWagePercentage = 0;
            float mtdTruckWage = 0;
            float mtdTruckRPH = 0;
            float mtdSGAWagePercentage = 0;
            ...
            //SG&A % ... need to get cells B10 & B13 from https://docs.google.com/spreadsheets/d/1ipm46yj2yD1s_28VL80gm4IpLu1s2XsV60tGSL1khKA/edit#gid=696941223
            MoveDailyWage m = new MoveDailyWage();
            m = gc.ConnectSheetsJunkSGAData("1ipm46yj2yD1s_28VL80gm4IpLu1s2XsV60tGSL1khKA");

            //Dump % ... need to get cell I16 from https://docs.google.com/spreadsheets/d/1aDIMguYytnNElZJxZQMq1b9534u_DVaRb0Ei_KacqbA/edit#gid=1071322922
            yesterdayDumpPercentage = gc.ConnectSheetsJunkDumpData("1aDIMguYytnNElZJxZQMq1b9534u_DVaRb0Ei_KacqbA");

            //Goals ... need to get data from https://docs.google.com/spreadsheets/d/1PY_Yy9NucCDBoPU2yE7vdE6yRSASgZpQLWldiFqxLEk/edit#gid=1829351603
            JunkGoals jg = new JunkGoals();
            jg = gc.ConnectSheetsJunkGoals("1PY_Yy9NucCDBoPU2yE7vdE6yRSASgZpQLWldiFqxLEk", firstOfMonthDate);
            ...
            cmd.Connection = dc.conn;
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.CommandText = "spAutomated_GetWagesByDatesV2";
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                else
                {
                    ...
                    //sga wage %
                    yesterdayIndirectWage = float.Parse(dt.Rows[0]["totalIndirectLabor"].ToString()) + float.Parse(m.StartDate.Replace("$","").Replace(",",""));
                    yesterdayIndirectWagePercentage = yesterdayIndirectWage / yesterdayRevenue;

                    //warehouse & support
                    yesterdayWarehouseWage = float.Parse(dt.Rows[0]["totalWarehouseLabor"].ToString());
                    yesterdayWarehouseWagePercentage = yesterdayWarehouseWage / yesterdayRevenue;

                    //truck rph
                    yesterdayTruckWage = float.Parse(dt.Rows[0]["totalTruckLabor"].ToString());
                    yesterdayTruckRPH = yesterdayRevenue / float.Parse(dt.Rows[0]["totalTruckHours"].ToString());

                    yesterdaySGAPercentage = yesterdayIndirectWage / yesterdayRevenue;

                    //overall rph
                    //yesterdayRPH = yesterdayRevenue / float.Parse(dt.Rows[0]["totalDirectHours"].ToString());
                    yesterdayRPH = yesterdayRevenue / float.Parse(dt.Rows[0]["totalRPHHours"].ToString());
                }
            }
            ...
            json = json + "  AJS: " + yesterdayAJS.ToString("C") + "  (" + jg.dailyAJS + ")\n";
            json = json + "  RPH: " + yesterdayRPH.ToString("C") + "  (" + jg.dailyRPH + ")\n";
            json = json + "  Truck RPH: " + yesterdayTruckRPH.ToString("C") + "  (" + jg.dailyTruckRPH + ")\n";
            json = json + "  Whse/Supp Wage: " + yesterdayWarehouseWagePercentage.ToString("P") + "  (" + jg.dailySupport + ")\n";
            json = json + "  Direct Wage: " + yesterdayDirectWagePercentage.ToString("P") + "  (" + jg.dailyDirectWage + ")\n";
            json = json + "  SG&A: " + yesterdaySGAPercentage.ToString("P") + "  (" + jg.dailySGA + ")\n";

            if (yesterdayDumpPercentage == -1) { json = json + "  Dumps: ERRROR\n"; } else { json = json + "  Dumps: " + yesterdayDumpPercentage.ToString("P") + "  (" + jg.dailyDump + ")\n"; }
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                else
                {
                    ...
                    //sg&a wage %
                    mtdIndirectWage = float.Parse(dt.Rows[0]["totalIndirectLabor"].ToString()) + float.Parse(m.EndDate.Replace("$", "").Replace(",", ""));
                    mtdIndirectWagePercentage = mtdIndirectWage / mtdRevenue;

                    //warehoues / support %
                    mtdWarehouseWage = float.Parse(dt.Rows[0]["totalWarehouseLabor"].ToString());
                    mtdWarehouseWagePercentage = mtdWarehouseWage / mtdRevenue;

                    //truck rph
                    mtdTruckWage = float.Parse(dt.Rows[0]["totalTruckLabor"].ToString());
                    mtdTruckRPH = mtdRevenue / float.Parse(dt.Rows[0]["totalTruckHours"].ToString());

                    mtdSGAWagePercentage = mtdIndirectWage / mtdRevenue;

                    //rph
                    //mtdRPH = mtdRevenue / float.Parse(dt.Rows[0]["totalDirectHours"].ToString());
                    mtdRPH = mtdRevenue / float.Parse(dt.Rows[0]["totalRPHHours"].ToString());
                }
            }
            ...
            //cmd.Parameters.Add("@segment", SqlDbType.NVarChar);
            //cmd.Parameters["@segment"].Value = "gjGoalAJS";
            ...
            //if (dt.Rows.Count > 0)
            //{
            //    goalAJS = float.Parse(dt.Rows[0]["displayName"].ToString());
            //}
            ...
            //cmd.Parameters.Add("@segment", SqlDbType.NVarChar);
            //cmd.Parameters["@segment"].Value = "gjGoalRPH";
            ...
            //if (dt.Rows.Count > 0)
            //{
            //    goalRPH = float.Parse(dt.Rows[0]["displayName"].ToString());
            //}
            ...
            json = json + "  Pace: " + mtdRevenuePace.ToString("C") + "  (" + jg.revenue + ")\n";
            ...
            json = json + "  AJS: " + mtdAJS.ToString("C") + "  (" + jg.monthlyAJS + ")\n";
            json = json + "  RPH: " + mtdRPH.ToString("C") + "  (" + jg.monthlyRPH + ")\n";
            json = json + "  Truck RPH: " + mtdTruckRPH.ToString("C") + "  (" + jg.monthlyTruckRPH + ")\n";
            json = json + "  Whse/Supp Wage: " + mtdWarehouseWagePercentage.ToString("P") + "  (" + jg.monthlySupport + ")\n";
            json = json + "  Direct Wage: " + mtdDirectWagePercentage.ToString("P") + "  (" + jg.monthlyDirectWage + ")\n";
            json = json + "  SG&A: " + mtdSGAWagePercentage.ToString("P") + "  (" + jg.monthlySGA + ")\n";
